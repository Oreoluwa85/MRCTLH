#Remote-blast "factory object" creation and blast-parameter initialization
  ...
  $Bio::Tools::Run::RemoteBlast::RETRIEVALHEADER{'ALIGNMENTS'} = 2;
  #remove a parameter
 ...
 getstore("http://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=protein&rettype=fasta&retmode=text&id=".join(",",qw(15223823,145360726)), 'temp.fasta');
  my $str = Bio::SeqIO->new(-file=>"temp.fasta" , -format => 'fasta' );
  ...
  ##while (my $input = $str->next_seq()){
    ...
    my $r = $factory->submit_blast('temp.fasta');
